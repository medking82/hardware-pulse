...
Desktop Mode's optional Auto Contrast samples three nearby screen pixels at most once every two seconds while the desktop is foreground and the readout is locked. These color samples are used locally to choose light or dark text; they are not stored or transmitted. Disable Auto Contrast to use a custom text color without background sampling.
