...
if($Installer){
    ...
    if($SigningCertificateThumbprint){& "$PSScriptRoot/Sign.ps1" -Path "$root/dist/HardwarePulse-0.5.3-Setup.exe" -Thumbprint $SigningCertificateThumbprint}
    Copy-Item "$root/dist/HardwarePulse-0.5.3-Setup.exe" "$root/dist/HardwarePulse-Setup.exe" -Force
    Get-FileHash "$root/dist/HardwarePulse-0.5.3-Setup.exe" -Algorithm SHA256 | Select-Object Hash,Path
}
