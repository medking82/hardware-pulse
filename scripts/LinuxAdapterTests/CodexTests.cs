...
static class CodexTests {
    ...
    public static void Run(){
        ...
        try{
            ...
            using(var adapter=new LinuxCodexQuota(path,handler)){
            ...
            using(var native=(HttpClientHandler)typeof(LinuxCodexQuota).GetMethod("NewHandler",BindingFlags.NonPublic|BindingFlags.Static).Invoke(null,null))
                Check(!native.AllowAutoRedirect&&!native.UseCookies,"Actual production handler rejects redirects and ambient cookies");
            if(!OperatingSystem.IsLinux()){
                bool rejected=false;try{using(var adapter=new LinuxCodexQuota()){};}catch(PlatformNotSupportedException){rejected=true;}Check(rejected,"Default source rejects unsupported OS");
            }
        }finally{if(File.Exists(path))File.Delete(path);}
        Console.WriteLine("PASS Linux Codex file/HTTP adapter: synthetic credentials, bounds, safe errors, redirects, cancellation and disposal");
    }
}
