...
& "$PSScriptRoot/Run-Hidden.ps1" $compiler @('/nologo','/target:exe','/reference:System.Runtime.InteropServices.RuntimeInformation.dll',"/out:$output\WindowsAdapterTests.exe","/reference:$output\Pulse.Adapters.Windows.dll","/reference:$output\Pulse.Core.dll",(Join-Path $root 'scripts\WindowsAdapterTests.cs'),(Join-Path $root 'scripts\CodexQuotaTests.cs')) $root
if($NativeArm64){
