...
$arguments=@('/nologo','/target:exe','/platform:x64',"/out:$worker/WindowsCollectorTests.exe",'/reference:System.Web.Extensions.dll',"/reference:$worker/HardwarePulse.Collector.exe","/reference:$worker/Pulse.Core.dll","/reference:$worker/Pulse.Adapters.Windows.dll",[IO.Path]::GetFullPath((Join-Path $root 'scripts/WindowsCollectorTests.cs')))
