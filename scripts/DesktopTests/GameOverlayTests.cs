...
static class GameOverlayTests {
    ...
    public static void Run(string? output=null) {
        ...
        var hardware=new MonitorSnapshot("24%","8 / 32 GiB","0","0",true,true){Hardware=new(){state="LIVE",values=new(){{"cpu",55},{"gpuLoad",60},{"gpu",65},{"cpuFan",900},{"diskC",40},{"vcore",1.2}},usage=new(){{"vram",new(){used=3,total=8,percent=37.5}}}}};
        ...
        foreach(string expected in new[]{"FixtureGame","FPS 73","24%","55.0 °C","60.0 %","65.0 °C","8 / 32 GiB","900 RPM","40.0 °C","1.200 V","3.0 / 8.0 GB"})Check(message.Contains(expected),"Overlay omitted "+expected);
        hardware.Hardware.state="OFFLINE";
        string stale=options.Message(hardware,fps,new("en"));
        Check(!stale.Contains("55.0")&&!stale.Contains("3.0 / 8.0")&&stale.Contains('—'),"Overlay exposed stale hardware as live");
        hardware.Hardware.state="LIVE";
        ...
        try {
            ...
            if(OperatingSystem.IsWindows()) {
                ...
                var uiOptions=new GameOverlayOptions{Fps=false};settings.GameOverlay=uiOptions;
                var language=new UiLanguage("en");
                var controls=new GameOverlayPanel(language,uiOptions,uiFps,()=>Check(store.Save(settings),"UI preference save"));
                ...
                try {
                    ...
                    controls.GetVisualDescendants().OfType<ColorPicker>().Single(x=>x.Name=="GameOverlayBackground").Color=Color.Parse("#123456");
                    ...
                    controls.GetVisualDescendants().OfType<CheckBox>().Single(x=>x.Name=="GameOverlayFps").IsChecked=true;
                    Until(()=>uiFps.Current.Current=="73","Overlay FPS option did not start capture");
                    Check(!uiFps.Enabled&&!uiFps.Current.Enabled,"Overlay capture changed the independent Monitor FPS preference");
                    Check(((SolidColorBrush)controls.GetVisualDescendants().OfType<Border>().Single(x=>x.Name=="GameOverlayPreview").Background!).Color.A==64,"Preview does not share overlay opacity");
                    controls.GetVisualDescendants().OfType<CheckBox>().Single(x=>x.Name=="GameOverlayFps").IsChecked=false;
                    Until(()=>uiFps.Current.TargetWindow==(nint)123&&uiFps.Current.Current=="—","Disabling overlay FPS lost target tracking or retained metrics");
                    int captures=uiSource.Captures,calls=uiSource.Calls;
                    Until(()=>uiSource.Calls>calls,"Hardware-only session stopped polling the target");
                    Check(uiSource.Captures==captures,"Hardware-only session kept capturing frames");
                    uiFps.Enabled=true;Until(()=>uiFps.Current.Current=="73","Monitor FPS did not keep capture independently");
                    controls.GetVisualDescendants().OfType<CheckBox>().Single(x=>x.Name=="GameOverlayEnabled").IsChecked=false;
                    Check(uiFps.Enabled&&uiFps.Current.Current=="73"&&!uiFps.TrackGame,"Closing overlay interrupted Monitor FPS");
                    uiFps.Enabled=false;Until(()=>uiFps.Sampling.IsCompleted,"Disabling the final consumer leaked the shared session");
                    ...
                    var picker=controls.GetVisualDescendants().OfType<ColorPicker>().Single();
                    var pickerButton=picker.GetVisualDescendants().OfType<Button>().First();
                    pickerButton.BringIntoView();pickerButton.Focus();Check(picker.IsKeyboardFocusWithin,"Color picker is not keyboard reachable");
                    view.KeyPress(Key.Space,RawInputModifiers.None,PhysicalKey.Space," ");view.KeyRelease(Key.Space,RawInputModifiers.None,PhysicalKey.Space," ");Dispatcher.UIThread.RunJobs();
                    Check(pickerButton.Flyout?.IsOpen==true,"Keyboard did not open the color picker");
                    pickerButton.Flyout!.Hide();
                    foreach(string locale in new[]{"en","zh-CN","zh-TW"}) {
                        language.Select(locale);view.Width=240;view.Height=1000;view.FontSize=16;view.RequestedThemeVariant=Avalonia.Styling.ThemeVariant.Dark;Dispatcher.UIThread.RunJobs();
                        foreach(var text in controls.GetVisualDescendants().OfType<TextBlock>().Where(x=>x.IsEffectivelyVisible))
                            Check(text.Bounds.Width<=240,"Overlay Settings text overflows narrow width: "+text.Text);
                        if(output!=null){using var frame=view.CaptureRenderedFrame();frame!.Save(Path.Combine(output,"game-overlay-settings-"+locale+".png"),Avalonia.Media.Imaging.PngBitmapEncoderOptions.Default);}
                    }
