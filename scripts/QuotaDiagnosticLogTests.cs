...
static class QuotaDiagnosticLogTests {
    ...
    internal static void Run(string stateDirectory){
        ...
        foreach(string reason in new[]{"CLI timeout","CLI exit failure","Invalid response"}){attempt.FailureKind=reason;Check(Encoding.UTF8.GetString((byte[])project.Invoke(null,new object[]{attempt})).Contains("\"reason\":\""+reason+"\""),"CLI failure category lost");}attempt.FailureKind="HTTP response";
