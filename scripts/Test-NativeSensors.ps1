...
& "$PSScriptRoot/Run-Hidden.ps1" "$env:WINDIR/Microsoft.NET/Framework64/v4.0.30319/csc.exe" @('/nologo','/target:library',"/out:$root\build\native\Pulse.Core.dll",'/reference:System.Web.Extensions.dll',"$root\src\Native\Models.cs","$root\src\Native\SensorProfile.cs") $root
...
Copy-Item "$root/build/native/Pulse.Core.dll" $testRoot
$harness=@'
. "$PSScriptRoot/Sensors.ps1"
Add-Type -AssemblyName System.Web.Extensions
Add-Type -Path "$PSScriptRoot/Pulse.Core.dll"
