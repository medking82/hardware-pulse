...
namespace HardwarePulse {
    ...
    public sealed class LinuxCodexQuota:IDisposable {
        const int Limit=1048576;
        const string Endpoint="https://chatgpt.com/backend-api/wham/usage";
        readonly string path;
        readonly HttpClient client;
        public LinuxCodexQuota():this(DefaultPath(),NewHandler()){}
        public LinuxCodexQuota(string loginPath,HttpMessageHandler handler){
        ...
        async Task<object> RequestAsync(string token,string account,CancellationToken cancel){
            using(var request=new HttpRequestMessage(HttpMethod.Get,Endpoint)){
                ...
                request.Headers.UserAgent.ParseAdd("HardwarePulse/"+typeof(LinuxCodexQuota).Assembly.GetName().Version.ToString(3));
