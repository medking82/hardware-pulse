...
public sealed class DesktopUpdatePanel : StackPanel,IDisposable {
    ...
    public Button HomeAction {get;}=new(){Name="HomeUpdate",HorizontalAlignment=Avalonia.Layout.HorizontalAlignment.Stretch};
    ...
    public DesktopUpdatePanel(UiLanguage language,PreviewSettings settings,Action changed,bool demo=false,IUpdateClient? client=null,Version? version=null) {
        ...
        Name="DesktopUpdatePanel";Spacing=10;
        ...
        HomeAction.Click+=(_,_)=>{if(coordinator.Ready)Install();else Operation=coordinator.CanDownload?DownloadAsync():CheckAsync();};
    ...
    void Render() {
        ...
        try {
            ...
            bool failed=coordinator.StatusKey is "Update check failed; try again" or "Update download failed; try again" or "Installation canceled or failed; try again";
            string? caption=coordinator.Installing?language.T("Installing update…"):coordinator.Downloading?language.T("Downloading update")+" · "+coordinator.Progress+"%":
                coordinator.Checking?language.T("Checking for updates…"):failed?language.T("Retry update"):coordinator.Ready?language.T("Install and restart"):
                coordinator.CanDownload?language.T("Update to")+" "+coordinator.VersionText:null;
            HomeAction.IsVisible=supported&&caption!=null;HomeAction.IsEnabled=supported&&!coordinator.Busy;
            HomeAction.Content=caption;ToolTip.SetTip(HomeAction,language.T(StatusKey));
            Avalonia.Automation.AutomationProperties.SetName(HomeAction,caption??language.T("Check for updates"));
