...
public sealed class MonitorWindow : Window {
    ...
    public MonitorWindow(IMonitorSource source,bool smoke=false,bool start=true,PreviewSettingsStore? store=null,bool measure=false,Func<IDesktopFpsSource>? fpsFactory=null) {
        ...
        foreach(var control in new[]{Language.Set(liveMode,"Live"),Language.Set(maxMode,"Session Max"),Language.Set(details,"Details")}){ApplyModeStyle(control);control.Margin=new Thickness(0,0,6,6);modes.Children.Add(control);}
        ...
        quota.ShowAll=claude.ShowAll=antigravity.ShowAll=settings.QuotaFull;
        ...
        var frame=new Grid();frame.Children.Add(viewport);WindowChrome.AddResizeEdges(this,frame);Content=frame;
    ...
    static void ApplyModeStyle(Button control) {
        ...
        control.Template=new Avalonia.Controls.Templates.FuncControlTemplate<Button>((button,scope)=>{
            ...
            plate.Bind(Border.BorderBrushProperty,button.GetObservable(BorderBrushProperty));
            plate.Bind(Border.BorderThicknessProperty,button.GetObservable(BorderThicknessProperty));
            plate.Bind(Border.PaddingProperty,button.GetObservable(PaddingProperty));
            return plate;
        });
