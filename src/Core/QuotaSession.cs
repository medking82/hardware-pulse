...
namespace HardwarePulse {
    ...
    public sealed class QuotaSession:IDisposable {
        sealed class Slot {internal bool Enabled;internal bool RefreshQueued;internal CancellationTokenSource Cancel;internal Task<QuotaReading> Pending;internal DateTimeOffset Next;internal QuotaReading Reading;internal int Version;internal int PendingVersion;}
        ...
        public void Enable(string provider,bool enabled){var slot=slots[provider];if(slot.Enabled==enabled)return;slot.Enabled=enabled;slot.RefreshQueued=false;slot.Version++;if(slot.Cancel!=null)slot.Cancel.Cancel();slot.Reading=new QuotaReading{Provider=provider,Status="Refresh pending"};slot.Next=DateTimeOffset.MinValue;}
