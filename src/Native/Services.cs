...
namespace HardwarePulse {
    public sealed partial class Shell {
        ...
        void WireOverlay(){
            ...
            overlayTimer.Interval=TimeSpan.FromMilliseconds(500);overlayTimer.Tick+=delegate{UpdateOverlay();if(DesktopFpsActive)UpdateDesktopFps();};Click("RefreshGames",RefreshGames);Click("ResetFps",delegate{if(frames!=null)frames.Reset();});Control<ComboBox>("GamePicker").SelectionChanged+=delegate{if(refreshingGames)return;var item=Control<ComboBox>("GamePicker").SelectedItem as ComboBoxItem;state["processName"]=item==null?"":(string)item.Tag;QueueSave();StartOverlay();};RefreshGames();StartOverlay();WireFpsSwitches();
        }
        void RefreshGames(){
