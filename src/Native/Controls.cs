...
namespace HardwarePulse {
    public sealed partial class Shell {
        ...
        public void ShowSettings(bool show){settingsVisible=show;Control<ScrollViewer>("SettingsPage").Visibility=show?Visibility.Visible:Visibility.Collapsed;Control<Border>("SettingsToolbar").Visibility=show?Visibility.Visible:Visibility.Collapsed;foreach(string name in new[]{"CardScroll","MonitorControls","MonitorFooter","Status"})Control<FrameworkElement>(name).Visibility=show?Visibility.Collapsed:Visibility.Visible;Control<Button>(show?"Back":"Settings").Focus();if(!show&&loaded)RenderPanel();else ApplyDensity();ApplyMaterial();}
        ...
        string BackgroundHex(){string color=settings.Text("background","#202831");return System.Text.RegularExpressions.Regex.IsMatch(color,"^#[0-9a-fA-F]{6}$")?color:"#202831";}
        void ApplyMaterial(){
