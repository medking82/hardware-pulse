...
**[下载 Windows 稳定版 0.6.41](https://github.com/medking82/hardware-pulse/releases/download/v0.6.41/HardwarePulse-Setup.exe)**
...
当前版本为 **0.6.41**，改善 quota failure recovery 和 diagnostics：连续 rate limit 会逐步延长 retry 间隔，App 显示下次 retry；Claude 同一 login source 的成功 reading 可在十分钟内明确标为 `Cached`，过期数值显示未知。Antigravity failure 的安全 source/error 信息保留在 tooltip 和有大小限制的本地 diagnostics 中。保留现有 WPF glass UI 和 Local Contrast。[Recovery behavior 与限制](docs/CLAUDE-QUOTA-RECOVERY.md#bounded-cached-claude-observation-windows-0641)。可选的 experimental Claude Code status-line source 仍需要手动 setup。[Setup 与 rollback](docs/CLAUDE-STATUSLINE.zh-CN.md)。
...
可直接安装 0.6.41 覆盖现有版本，无需全新安装。安装程序会停止旧采集器、替换应用程序及其两个启动任务，并清理明确列出的旧脚本/源代码文件。偏好设置和桌面位置与大小保留在 LocalAppData；Windows PowerShell 和共享 PawnIO 保持安装。升级中断或失败时可能需要重新运行安装程序；文件清理不提供事务性回滚。
...
发行版中的 `HardwarePulse-Setup.exe`（版本 0.6.41）包含应用程序、固定版本的 LibreHardwareMonitor 库、许可声明/源代码归档，以及官方 PawnIO 2.2.0 前置依赖安装包，无需在运行时下载依赖项。目标 Windows 版本自带 .NET Framework 4.8 和 Windows PowerShell 5.1；安装程序会检查 .NET 要求。
