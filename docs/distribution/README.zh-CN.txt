...
Claude 和 Antigravity 具有独立 opt-in。Claude 读取已有 login；Antigravity 需要 Windows 当前用户运行中的 language server，其他平台会明确显示 unavailable。Pulse 不执行 login 或 token renewal。
preview 使用独立 settings，保存窗口大小、theme、network 选择和各 quota provider 的 opt-in。
...
设置 → 桌面可调整文字大小、置顶、背景不透明度及系统模糊，并保存选择和浮动窗口大小、位置。不透明度只影响背景，不影响文字；模糊状态反映实际后端效果，强度由系统控制。
已启用的 quota provider 复用各自现有 session 显示在 floating monitor；关闭一个不会影响其他 provider。锁定后隐藏编辑控件，重新打开后恢复。
Windows/macOS 及具有 XFixes 的 Linux X11 可请求原生鼠标穿透锁定，从主界面或托盘、菜单栏重新打开即可解锁。X11 锁定时临时移除可交互的窗口边框，解锁后恢复。Windows ARM64 交互验证尚未完成，不包含在本次预览更新中。尚未实现原生 Wayland 锁定。

限制
这是实验性预览版，尚未与 Windows 稳定版实现完整功能对等。
尚未接入 GPU 遥测、FPS、完整桌面悬浮模式、可调模糊半径和局部自适应对比度、全局快捷键、
开机启动及自动更新。
