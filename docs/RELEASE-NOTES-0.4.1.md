...
**Update limitation:** Check for Updates currently checks stable GitHub releases only. It does not discover this Pre-release. Download Update opens a release page; it does not download or install automatically. Use this release's EXE asset to update manually.
...
**目前不能自动下载安装。** Check for Updates 只检查 stable release，不会发现本 Pre-release；Download Update 只打开 GitHub 页面。请从本页 Assets 下载 EXE 手动更新。installer 仍为 self-signed，SignPath 尚未获批。
