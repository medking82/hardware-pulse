...
| Desktop integration | Floating geometry, lock, topmost, tray, opacity, colors; Windows configurable global shortcut with conflict feedback and same-window Desktop show/hide. Locked Windows readout uses desktop Z-order with explicit editable unlock; native tests cover topmost, hidden restore and disposal | Show Desktop/Explorer restart and installed placement acceptance; background sampling, Local Contrast and screenshot behavior; repeat shortcut interaction on the final installed build |
