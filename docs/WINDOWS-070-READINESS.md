...
| Lifecycle | Close-to-tray, one sampler after restore, and Windows per-user/session single instance. Real second-process tests cover activation and graceful/crash recovery; shortcut conflicts are explicit | Startup/restore mode and final installed-launch acceptance |
