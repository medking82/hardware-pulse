...
| FPS | Shared Monitor/Desktop client, opt-in, app selection and Reset reuse the existing protocol and process identity policy; a dedicated worker is buildable. Game overlay shares the same session target/metrics, current hardware snapshots, independent metric switches, six anchors and appearance controls; native fixtures cover foreground/minimize hiding, restore and pass-through | Install/register the matching worker; verify actual capture, real-game overlay behavior, display configurations and total resource cost |
...
| Lifecycle | Close-to-tray, one sampler after restore, Windows per-user/session single instance, and Monitor/Desktop/Tray startup modes. Real lifetime tests verify hidden startup without Monitor flash, tray fallback, activation precedence and exit cleanup. Startup controls invoke only the matching protected worker, read back state and reject development launches; fake tests cover UAC cancellation and late results | Actual installed startup enable/disable/collector commands and final logon/session-shutdown acceptance |
| Updates/distribution | Separate preview archives. Shared update UI reuses the existing stable release/installer selection, bounded transport, digest verification and explicit installation owner; preview/development transport is disabled. Modern parser/UI and legacy updater regression checks cover the shared wiring | Final Windows stable installer and matching update asset, settings migration policy, actual verified download/install/restart, install/uninstall and rollback verification |
