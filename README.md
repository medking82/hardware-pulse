...
**[Download 0.6.38 EXE](https://github.com/medking82/hardware-pulse/releases/download/v0.6.38/HardwarePulse-Setup.exe)** · [0.6.38 release notes](https://github.com/medking82/hardware-pulse/releases/tag/v0.6.38)
...
Latest: **0.6.38** retries Claude `Login required` after 30 seconds, then backs off through 60, 120, 240 and 300 seconds while the credential owner recovers. Successful reads return to five-minute refreshes; rate-limit deadlines remain enforced. This improves recovery detection, not token renewal. The WPF glass UI and previous performance improvements are preserved. [Recovery behavior and validation limits](docs/CLAUDE-QUOTA-RECOVERY.md#bounded-owner-login-recovery). The optional experimental Claude Code status-line source requires manual setup and expires stale snapshots. [Setup and rollback](docs/CLAUDE-STATUSLINE.md).
...
Settings in 0.6.38: Settings → AI Quota enables independent Codex,
...
Version 0.6.38 also displays negotiated Network Link Speed for the selected
...
The published version is **0.6.38** with multilingual UI and animated card reordering. Use the download link above for the latest installer.
...
The release asset `HardwarePulse-Setup.exe` (version 0.6.38) bundles the application, pinned LibreHardwareMonitor libraries,
