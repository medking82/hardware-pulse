...
**[Download 0.6.10 / 下载 0.6.10 EXE](https://github.com/medking82/hardware-pulse/releases/download/v0.6.10/HardwarePulse-Setup.exe)** · [0.6.10 release notes](https://github.com/medking82/hardware-pulse/releases/tag/v0.6.10)
...
Latest: **0.6.10** adds optional local adaptive text contrast under Settings → Desktop, removes the outer frame at 0% background opacity, and adds a Desktop FPS reading without requiring the separate FPS overlay. Local contrast may exclude Pulse from screenshots and recordings. / 新增局部文字自适应对比度，修复透明外框，并支持独立 Desktop FPS reading；局部对比度开启时，截图及录屏可能不包含 Pulse。
...
Settings in 0.6.10: Settings → AI Quota enables independent Codex,
...
Version 0.6.10 also displays negotiated Network Link Speed for the selected
...
The published version is **0.6.10** with multilingual UI and animated card reordering. Use the download link above for the latest installer.
...
The release asset `HardwarePulse-Setup.exe` (version 0.6.10) bundles the application, pinned LibreHardwareMonitor libraries,
...
当前 version 为 **0.6.10**，包含 multi-language UI。使用上方 download link 获取最新 installer。
