...
**[Download 0.6.41 EXE](https://github.com/medking82/hardware-pulse/releases/download/v0.6.41/HardwarePulse-Setup.exe)** · [0.6.41 release notes](https://github.com/medking82/hardware-pulse/releases/tag/v0.6.41)
...
Latest: **0.6.41** makes quota failures easier to recover from and diagnose. Repeated rate limits back off progressively; App shows the next retry, and recent same-source Claude readings can remain explicitly `Cached` for less than ten minutes. Expired values become unknown. Antigravity failures retain safe source/error details in tooltips and bounded local diagnostics. Existing WPF glass styling and Local Contrast remain intact. [Recovery behavior and limits](docs/CLAUDE-QUOTA-RECOVERY.md#bounded-cached-claude-observation-windows-0641). The optional experimental Claude Code status-line source still requires manual setup. [Setup and rollback](docs/CLAUDE-STATUSLINE.md).
...
Settings in 0.6.41: Settings → AI Quota enables independent Codex,
...
Version 0.6.41 also displays negotiated Network Link Speed for the selected
...
The published version is **0.6.41** with multilingual UI and animated card reordering. Use the download link above for the latest installer.
...
The release asset `HardwarePulse-Setup.exe` (version 0.6.41) bundles the application, pinned LibreHardwareMonitor libraries,
