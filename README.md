...
**[Download 0.6.42 EXE](https://github.com/medking82/hardware-pulse/releases/download/v0.6.42/HardwarePulse-Setup.exe)** · [0.6.42 release notes](https://github.com/medking82/hardware-pulse/releases/tag/v0.6.42)
...
Latest: **0.6.42** gives the Antigravity CLI a bounded 25-second outer deadline around its 15-second command timeout, allowing startup and output completion time. Safe diagnostics distinguish CLI timeout, unsuccessful exit and invalid response. Desktop-first selection, hidden execution, account boundaries and existing WPF glass styling remain intact. [CLI deadline behavior and evidence](docs/QUOTA-INTEGRATION.md#cli-deadline-margin-windows-0642). The previous quota recovery improvements remain available, including progressive rate-limit backoff and short-lived same-source Claude readings explicitly marked `Cached`. The optional experimental Claude Code status-line source still requires manual setup. [Setup and rollback](docs/CLAUDE-STATUSLINE.md).
...
Settings in 0.6.42: Settings → AI Quota enables independent Codex,
...
Version 0.6.42 also displays negotiated Network Link Speed for the selected
...
The published version is **0.6.42** with multilingual UI and animated card reordering. Use the download link above for the latest installer.
...
The release asset `HardwarePulse-Setup.exe` (version 0.6.42) bundles the application, pinned LibreHardwareMonitor libraries,
