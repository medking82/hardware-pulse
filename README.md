...
**[Download 0.5.2 / 下载 0.5.2 EXE](https://github.com/medking82/hardware-pulse/releases/download/v0.5.2/HardwarePulse-Setup.exe)** · [0.5.2 release notes](https://github.com/medking82/hardware-pulse/releases/tag/v0.5.2)
...
Latest: **0.5.2** reduces repeated UI allocations and skips hidden-window card rendering while preserving Session Max and sensor polling. It includes the 0.5.1 startup registration fix. / 减少 UI allocations 和隐藏窗口 rendering，保留原有 readings 与 startup fix。
...
The published version is **0.5.2** with multilingual UI and animated card reordering. Use the download link above for the latest installer.
...
The release asset `HardwarePulse-Setup.exe` (version 0.5.2) bundles the application, pinned LibreHardwareMonitor libraries,
...
当前 version 为 **0.5.2**，包含 multi-language UI。使用上方 download link 获取最新 installer。
